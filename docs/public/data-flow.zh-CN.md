...
| 第三方 GPT 的独立搜索 | 仅 ChatGPT Codex 后端 | 搜索结果可进入后续模型上下文 |
...
订阅 bearer 和账号 Header 只存在于固定官方链路；第三方 API Key 只存在于对应 Provider 链路。`/v1/alpha/search` 不开放，也不会把本地 API Key 升级为订阅身份。
