...
`history recover --thread THREAD_ID [--source ROLLOUT.jsonl]` is a metadata-only preview by default. Add `--yes` only after Codex App is closed and the Gateway is idle. It writes exact portable checkpoints only when the complete rollout lineage, ordinal boundaries, compaction items, and tool call/results are losslessly verifiable. See [fork and compacted-history recovery](compaction-recovery.md).
