...
Official request fields such as `access_programs` remain byte-transparent on this path. If the signed-in organization is not entitled to a requested program, the official error is forwarded unchanged; the Router does not remove the field or retry with different capabilities.
...
Codex owns compaction decisions. Native official compaction triggers and opaque compaction items stay on the transparent relay; their presence alone does not require Engine adaptation. A compact response replaces the active context window rather than appending the pre-compaction history. The archive keeps the immutable original, the effective replacement window, and a target-portable view as separate records. Gateway-owned checkpoints, retained Engine response IDs and cross-provider migration still require history replay. Successful observation commits are ordered per branch; a later switch waits for the bounded commit even when the client omits `previous_response_id`. Missing or contradictory lineage does not block ordinary official continuation, but it cannot authorize cross-provider replay. See the [OpenAI compaction contract](https://developers.openai.com/api/docs/guides/compaction).

The router executes `native`, `summary`, or `unsupported` behavior declared by the source target. Same-account, same-target `native` compaction stays channel-owned: the Router forwards the Codex compaction request and later opaque checkpoint without choosing a summary window or blocking it on a token estimate. `summary` is an explicit, lossy Gateway-owned mode and is never selected as a fallback for failed or unverified native compression. Cross-target migration is separate: `nativeMigrationSummary` authorizes one tool-free source-model summary only when exact portable history cannot be used by the destination. The summary is stored with its source-window hash; it never repairs a gap, swallows the necessary tail, recursively summarizes, retries, changes model, or changes channel. Fork recovery follows verified `history_base` boundaries with bounded depth, preserves unknown tool execution as an explicit historical uncertainty, and stores shared event prefixes instead of copying the complete history into every checkpoint. HTTP SSE from either the Relay or Engine uses the shared [inactivity timeout](configuration.md#request-timeouts), not a total generation deadline.
