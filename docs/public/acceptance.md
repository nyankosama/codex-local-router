...
The configuration-space and v0.4.0 third-party standalone-search reports are frozen historical records, not statements about the currently installed service: [configuration spaces](configuration-spaces-acceptance.md) and [third-party OpenAI search](third-party-openai-search-acceptance.md). Their protocol evidence remains useful, while App UI, local activation and each new release are signed off separately.
