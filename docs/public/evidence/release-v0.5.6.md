# v0.5.6 release qualification evidence

> **Frozen evidence**  
> Applies to: `v0.5.6` release qualification and its accumulated acceptance lineage.  
> This immutable record does not describe the current release, installed service, or Provider health.

The configuration-space and v0.4.0 third-party standalone-search reports are frozen historical records, not statements about the currently installed service: [configuration spaces](configuration-spaces-v0.3.0.md) and [third-party OpenAI search](third-party-openai-search-v0.4.0.md). Their protocol evidence remains useful, while App UI, local activation and each new release are signed off separately.
...
The historical configuration-space candidate gate intentionally performed zero real model calls and did not run `e2e:focused -- --run`. Its recorded environment kept the Router and switcher unloaded, port 8788 closed, integration disabled, and the real Codex config on the built-in OpenAI provider. Those facts describe that run only; App-server coverage never replaces a later App UI sign-off.
...
The deterministic profile stage adds five local HTTP/WS, tool, lifecycle and performance cases. The entire real gate is capped at five turns, seventeen model generations and nine searches. The third-party stage is capped at `3 / 8 / 3`, including its single client-owned MCP search and the four sends required by a deferred MCP call/result closure; the official stage is capped at `2 / 9 / 6`. Current Codex 0.155 search continuation may legitimately require one additional unique model send across the cached/live pair; the zero-retry and duplicate-payload rules are unchanged. Repeated payloads, blocked requests, implicit retries, a missing case, a different Codex binary or commit, incomplete MCP inventory, credential crossing, or any non-PASS stage rejects the tag. Stages run sequentially and stop after the first failure, so a local defect cannot consume the remaining external budget.
